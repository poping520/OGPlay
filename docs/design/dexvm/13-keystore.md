...
日期：2026-09-16。状态：约定范围内验收完成；KS-01..09 统一归档于 DVM-172。
...
实际状态统一见 DVM-172；KS 单元仍保留独立验收结果，不因任务单合并而删除验收范围。
...
已实现范围为 BKS v2 写出、v0/v1/v2 读取、证书、AES RAW、
RSA/EC PKCS#8 私钥及标准/Broken/Old key PBE；独立 `OGPlayKeyStore` Provider 已注册并设为
默认 BKS。固定 API 19 双向互操作和历史隔离 oracle 证据见 DVM-172 与能力清单。KS-03 的
CallbackHandler 实际取密码、同 store 双线程及 KS-08 的磁盘跨 session 重载、应用隔离回归
已闭合；验收范围限于本文约定的软件 KeyStore 与算法集合。

DSA、X.509 public-key entry、PKIX、系统 CA、TLS、AndroidKeyStore、Binder 和硬件密钥仍不在
已发布范围；恢复私钥不代表签名生成或 TLS 客户端认证。
