...
3. **保留后缀**：`.__ogplay_tombstone__` 与 `.__ogplay_tmp__` 为实现保留；
   ...
   命中即值得记录）。
4. **大小写**：VFS 索引维持 ASCII 大小写不敏感、保留首见拼写的既有规则；
   落盘用首见拼写。装载时若宿主目录内出现大小写折叠冲突（Linux/macOS 上
   人为制造），沿用现有不变量：发布挂载前明确失败。
...
默认根目录解析放在 `frontend`（它已负责 `--system-dir`/`--external-dir` 的
宿主路径策略）；`runtime/vfs` 只接受一个已存在或可创建的绝对目录，不做
平台约定推断，维持依赖方向。
