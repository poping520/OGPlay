...
docs/
  ...
  adr/          架构决策记录（只追加，不修改）
  modules/      各模块契约的汇总索引
  playbook/     开发方式手册（适配、测试、排查的操作步骤，持续追加）
...
决策按主题保存于 [`docs/adr/`](../adr/README.md)，**历史正文只追加不修改**。新决策在
对应主题末尾追加，编号全局递增，不再为每条决策单独建文件。要推翻旧决定，追加新编号
并标注精确的 `Supersedes`；索引与引用使用 `主题.md#adr-NNNN`。

模板（将 NNNN 替换为下一个全局编号）：
...
- 日期：YYYY-MM-DD
- 关联：相关任务或模块契约
- Supersedes：仅在替代旧决策时列出原编号与具体条款

### 背景

为什么现在需要作出决定，以及约束和备选方案。
