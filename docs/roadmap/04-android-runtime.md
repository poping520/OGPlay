...
**结论：完整冻结 233 槽 ABI，但函数行为按覆盖率分批实现。** M3 优先完成引用、异常、
类/字段/方法查找与常用调用变体、字符串、数组、`RegisterNatives`，以及 JavaVM 的
线程附着接口；低频反射、直接缓冲区等随后按累计样本证据推进。所有未实现槽位始终
必须“记账 + 显式 trap”，绝不能静默返回 0，也不能因为暂缓实现而从 ABI 表中删除。
...
**M8 实证修正**：上表的"否"只对"游戏逻辑是否在 Java"成立，对兼容成本不成立。
Gameloft 这类"极薄"title 的 Java 胶水层（生命周期、license/billing/analytics、
音频转发）在 profile 路线下仍需逐 title 人工逆向为 `[[java.class]]` 映射与
`native_call` 序列：Asphalt 6 因漏抄一次 `nativeInit` 在远端调用点爆错
（ADR-0017 背景）；Dungeon Hunter 单次点击命中缺失 handler，对账剩 13 个
待反编译取证（WU-M8-011）。**薄层 title 同样受益于 L2**——经方法级接管
（design/dexvm 04 §1）解释执行胶水方法体，人工翻译成本归零。
