...
更新：2026-08-18 · APK Startup 阶段 A 已完成；APS-1..4 已交付
...
- **APK Startup**：APS-1 已发布 Manifest Application/launcher facts；APS-2 已建立
  ...
  DexVM `System.load*` 接线与 frontend 切换尚未实施。
...
- Windows/x64 `windows-msvc`：792/792 CTest。
- macOS/arm64 最近记录：766/766 CTest。
- Windows 预设使用原生核数并行工程；OGPlay 自有 MSVC target 启用 `/MP`。

## 下一步

1. 实施 APS-5 DexVM `System.load/System.loadLibrary` 与重入接线。
