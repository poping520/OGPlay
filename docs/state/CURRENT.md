...
更新：2026-08-04 · M4 GLES IDL 基础设施完成

## 当前阶段

- M0、M1、M2、M3 均已完成；M4 图形栈正在开发。
- `WU-0160` 已建立严格 GLES JSON IDL、确定性 C++ 目录生成器及构建期漂移检查；
  下一个任务编号为 `WU-0161`。
...
1. `WU-0161` 补齐 GLES2 core 入口目录并与 ANGLE `gl2.h` 自动核对。
2. 随后接入 guest 内存搬运、生成式分派，再增加窗口 surface 与 present。
