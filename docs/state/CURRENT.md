...
更新：2026-08-16 · M10 Layout UI LUI-1..5 已完成，typed AXML、UiTree、DexVM binding、
registry inflater/merge 与 FrameLayout traversal 已闭合；macOS SDL 事件泵已隔离 DexVM GLThread；DVM-31 EGL façade 实现完成，
...
- **M10 Layout UI LUI-1..5 已完成**：compiled AXML 已是 generic typed 输出；
  ...
  registry inflater 正确 attach 普通 root/`<merge>`，unknown structural 输入事务失败；
  Activity switch 清理旧 content generation。MeasureSpec + FrameLayout 已让 synthetic root
  以 surface exact constraint 统一解析 fixed/match/wrap、margin/padding、gravity 与 screen
  geometry。下一步 LUI-6 horizontal LinearLayout 与 drawable intrinsic measure。
...
能力现状以 `capabilities.toml` 为准。macOS/arm64 最近 full CTest 726/726；
