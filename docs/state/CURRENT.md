...
更新：DVM-100 已补齐 `System.getSecurityManager()`，并将 API 19
`SecurityManager` 纳入 BootDex；PvZ 当前首错为 `DateFormat.format(Date)`。
BootDex 已扩展为 47 个 class_def。
...
- **BootDex**：Boot/Application DexUnit、unit-local 常量池、精确 intrinsic method overlay
  ...
  pinned AOSP jar 双次重组 canonical JAR；当前含 EnumSet 11 类、Executors fixed-pool
  相关 35 类及 `SecurityManager`。Unsafe 原语已补齐，并行线程池语义尚未闭合。
...
- **SecurityManager**：`System.getSecurityManager()` 依 API 19 固定返回 null，类形状
  来自 BootDex；不安装 permission manager 或接入宿主安全机制。
- **DexVM**：稳定 linker metadata、`MethodShape`、owner state/GC、反射基础、guest 线程、
  ...
  生产默认关闭。
- **Title**：PvZ 已越过 `Executors$DefaultThreadFactory.<init>`，当前缺少
  `DateFormat.format(Date)`；Tales 首错为
...
- 2026-09-05 macOS Release：SecurityManager BootDex、System overlay、Unsafe/EnumSet 与
  架构定向检查 5/5；builder 和 payload 校验通过。PvZ 关闭 survey 实跑越过
  `System.getSecurityManager()`，首错推进到 `DateFormat.format(Date)`。
...
1. 继续处理 PvZ 日志使用的 DateFormat 格式化边界，再推进 NetworkImpl 与 Tales
