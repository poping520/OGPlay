...
更新：2026-08-05 · M4 综合样例确定性输入回归

## 当前阶段

- M0、M1、M2、M3 均已完成；M4 图形栈正在开发。
- `WU-0191` 已以真实渲染像素闭合综合样例的键盘换色和指针标记移动；下一编号为
  `WU-0192`，准备 M4 软件后端黄金帧与出口验证矩阵。
...
1. 准备 M4 软件后端黄金帧与三平台出口验证矩阵；Linux/macOS 在对应平台机器本地增量
   构建并验收，样例未使用的 GLES2 槽仍按游戏需求逐步绑定。
