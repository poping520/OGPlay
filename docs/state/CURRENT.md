...
更新：2026-08-04 · M4 GLES2 core 目录完成

## 当前阶段

- M0、M1、M2、M3 均已完成；M4 图形栈正在开发。
- `WU-0161` 已把 GLES2 core 142 个入口和 397 个参数纳入 IDL，并与固定 ANGLE
  `gl2.h` 自动核对；下一个任务编号为 `WU-0162`。
...
1. `WU-0162` 实现基于强类型 guest 地址和 `MemoryBus` 的受检输入/输出搬运层。
2. 随后接入生成式分派，再增加 GLES1、窗口 surface 与 present。
