...
class MacosVirtualMemory final : public VirtualMemoryReservation {
public:
    explicit MacosVirtualMemory(const std::uint64_t size) : size_(size) {
...
std::unique_ptr<VirtualMemoryReservation> ReserveVirtualMemory(const std::uint64_t size) {
    return std::make_unique<MacosVirtualMemory>(size);
}
