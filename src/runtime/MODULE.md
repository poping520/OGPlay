...
- `JniFieldStore`：按 descriptor 生成 Java 默认值并隔离实例字段，静态字段按声明共享；
  访问统一检查字段类别、对象类兼容性和精确值类型；可安装 session-lifetime 窄 hooks，
  让 DexVM 自有字段委托给解释器对象/静态槽，未接管字段仍使用 HLE side table。
...
- 字段值必须与声明 descriptor 精确匹配；实例字段按对象身份隔离，继承到子类的静态字段
  仍只有声明对应的一份存储。DEX 声明字段跨 JNI 时必须与解释器共享槽，引用读取发布
  当前线程 local reference，写入解析回同一对象身份；不得复制到 `JniFieldStore`。
