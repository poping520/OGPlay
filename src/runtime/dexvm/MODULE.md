...
- `IoRuntime`（DVM-79/DVM-91）：统一拥有 java.io input/output bytes、cursor、close 与 wrapper
  ...
  工作目录、writable 状态、文件 rename 和单级/递归建目录同样是注入事实：相对 `File`
  不得读取宿主 cwd，`mkdir()` 不得退化为递归创建；core 不自行读取或修改宿主权限位。
...
- `VmMonitorTable`（`vm_monitors.h`，`Interpreter::Monitors()`）：session 级
  ...
  timed wait 不得停泊在唯一能推进确定性 Clock 的驱动线程上：根上下文（token 1，
  `kRootLifecycleToken`）总可在有界 peer 窗口后，经 `SetClockAdvance` 快进到自身
  deadline；worker 默认仍等待帧泵，只有 `SetClockDriverBlockedProbe` 明确证明驱动
  线程正在同步阻塞时才可代为推进。多个停泊者的推进串行化，并只补当前 deadline
  与 Clock 的差值，避免累计超调；peer 的 notify/interrupt 仍可先赢。未发布快进
  钩子的会话（测试接自走 wall clock）保持原停泊语义。
...
- `VmThreadRuntime`（`vm_threads.h`）：一个 guest Java 线程 = 一个
  ...
  Clock；timed join 与 `Sleep` 共用上述 deadline 协调：根线程可快进，worker
  通常等帧泵，只在 clock-driver-blocked probe 为真时补到自身 deadline。
  `Interrupt` 只写 monitor execution-token interrupt state 并唤醒
