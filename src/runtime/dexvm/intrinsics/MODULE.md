...
hierarchy：Android 4.4.4 的完整家族统一位于 `java_lang_throwables.cpp`。每个
Java class 仍保留独立 `Declare_*()`，但这些函数为 TU-private；该文件只向
`catalog.h` 暴露一个 `AppendJavaLangThrowables()` family 入口，
`catalog.cpp` 不逐类感知 Throwable hierarchy。后续新增 API-family 聚合文件
沿用同一规则：类级 `Declare_*()` 留在所属 TU 内，只暴露单一 family append
入口。该 API-family 聚合文件为控制翻译单元数量允许超过通常 800 行限制。
