...
void Measure(UiTree& tree, const UiNodeId id, const MeasureSpec width_spec,
    ...
    for (const auto child_id : node.children) {
        auto& child = *tree.Get(child_id);
        if (child.visibility == Visibility::Gone) continue;
        if (child.layout.width.mode == SizeMode::MatchParent ||
...
void LayoutFrame(UiTree& tree, const UiNodeId id, const std::int32_t screen_x,
                 const std::int32_t screen_y) {
    auto& parent = *tree.Get(id);
    parent.screen_frame = {screen_x, screen_y, screen_x + parent.measured.width,
                           screen_y + parent.measured.height};
    ...
    for (const auto child_id : parent.children) {
        ...
        LayoutFrame(tree, child_id, screen_x + left, screen_y + top);
    }
}
...
void LayoutUiTree(UiTree& tree, const UiMetrics metrics) {
    ...
    LayoutFrame(tree, tree.Root(), 0, 0);
    tree.ClearDirty();
}
