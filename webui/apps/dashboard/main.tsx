...
const sectionNames: Record<string, string> = {session: '会话', diagnostics: '运行时诊断', gpu: 'GPU · GLES', vfs: 'VFS · IO', audio: '音频', capabilities: '能力账本', log: '结构化日志'};
  ...
  useEffect(() => {
    ...
    const plot = chart.current = new uPlot({width: Math.max(240, element.clientWidth), height: 190,
      ...
      hooks: {setCursor: [(p) => {
        ...
        if (frame != null && String(frame) !== store.selection.frame) store.select({...store.selection, frame: String(frame)});
      }]},
    }, [[], []], element);
...
function App() {
  ...
  const latest = store.latest, session = latest?.session?.status !== 'unavailable' ? latest?.session?.data : null;
  const rows = threads(latest);
  const focused = threads(store.selectedThread).find(t => String(t.guest_tid) === store.selection.guestTid);
  ...
  return <>
    ...
    <main>
      <aside class="panel topology"><div class="panel-title"><h2>模块拓扑</h2><span>数据来源</span></div><div class="layer-label">编排 → 运行时 → 边界</div>
        {Object.entries(sectionNames).map(([name, title]) => { const s = latest?.[name] as Section | undefined; return <button class={`module ${module === name ? 'selected' : ''}`} onClick={() => setModule(name)}>
      ...
      <section class="center">
        <div class="panel"><div class="panel-title"><h2>帧进度</h2><span>{store.history.length} / 600 个采样 · {store.connection === 'connected' ? '实时' : '历史'}</span></div>
          <Timeline history={store.history}/><p class="chart-note">悬停选择 frame；采样曲线不等同于逐帧耗时。当前事件来源没有 frame 时不会强行关联。</p></div>
        ...
        <div class="panel"><div class="panel-title"><h2>事件流</h2><span>{events.length} 条 · {store.connection === 'connected' ? '实时' : '历史'}</span></div>
          ...
          <div class="events table-scroll"><table><thead><tr><th>Sequence</th><th>类型</th><th>Thread</th><th>Frame</th><th>事实</th></tr></thead><tbody>
            {events.slice(-100).reverse().map(e => <tr><td>{e.sequence}</td><td><span class="event-kind">{e.kind}</span></td><td>{e.guest_tid ?? '—'}</td><td>{e.frame ?? '未关联'}</td><td>{e.method ?? e.lifecycle_phase ?? e.detail ?? (e.syscall_nr == null ? '—' : `syscall ${e.syscall_nr} → ${e.result}`)}</td></tr>)}
      ...
      <aside class="right"><div class="panel focus"><div class="panel-title"><h2>联动焦点</h2><span>{store.selection.guestTid ? `#${store.selection.guestTid}` : '未选择'}</span></div>
        ...
        <div class="body"><p>{section ? reasonText(section.reason) : '等待当前快照'}</p>
          ...
          <p class="muted">完整模块面板在后续阶段接入。当前视图只展示已有事实。</p></div></div></aside>
    </main><footer>OGPLAY · DASHBOARD<span>schema 1 · 固定容量历史 · 不推进 guest</span></footer>
  </>;
}
